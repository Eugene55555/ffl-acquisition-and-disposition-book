...
const COPY: Record<Locale, Copy> = {
  ...
  ru: {
    heading: 'Отзывы читателей',
    lede: 'Честные заметки дилеров, оружейников и коллекционеров, которые ведут эти журналы у прилавка. Без регистрации и без выпрашивания звёзд — только то, что написали покупатели.',
    emptyTitle: 'Опубликованных отзывов пока нет',
    emptyBody:
      'Первый отзыв — самый трудный. Если один из этих журналов у вас на столе, расскажите другим держателям FFL, каково это в ежедневной работе: где раскладка помогла, а где нет.',
    emptyCta: 'Оставить первый отзыв',
    emptyNote: 'Отзывы читает человек и публикует вручную — обычно в течение пары дней.',
    writeCta: 'Оставить отзыв',
    summaryHeading: 'Сводка рейтинга',
    basedOn: (n) => (n === 1 ? 'на основе 1 отзыва' : `на основе ${n} отзывов`),
    noRatingYet: 'Оценок пока нет',
    distributionLabel: 'Распределение оценок',
    barAria: (stars, count) => `${stars} звёзд: ${count}`,
    ratingValue: (value) => `${value} из 5`,
    searchLabel: 'Поиск по отзывам',
    searchPlaceholder: 'например, серийный номер',
    bookFilterLabel: 'Фильтр по книге',
    allBooks: 'Все книги',
    sortLabel: 'Сортировка',
    sort: { newest: 'Сначала новые', helpful: 'Самые полезные', highest: 'Сначала высокие', lowest: 'Сначала низкие' },
    resultsCount: (n) => (n === 1 ? 'Показан 1 отзыв' : `Показано отзывов: ${n}`),
    resultsNone: 'По вашим фильтрам отзывов нет',
    verified: 'Проверенная покупка',
    helpful: 'Полезно',
    helpfulThanks: 'Спасибо',
    replyFrom: 'Ответ автора',
    showMore: 'Показать ещё',
    showingCount: (shown, total) => `Показано ${shown} из ${total}`,
    photosAlt: (name) => `Фото к отзыву: ${name}`,
    noResultsTitle: 'Ничего не нашлось',
    noResultsBody: 'Попробуйте короче запрос, другую книгу или сбросьте фильтры.',
    clearFilters: 'Сбросить фильтры',
    modalTitle: 'Оставить отзыв',
    modalIntro:
      'Аккаунт не нужен. Отзыв уходит владельцу сайта и публикуется вручную — поэтому он появится не сразу, обычно в течение дня-двух.',
    nameLabel: 'Ваше имя',
    namePlaceholder: 'Даниил Р. (или псевдоним)',
    locationLabel: 'Город, штат (необязательно)',
    locationPlaceholder: 'Reno, NV',
    bookLabel: 'Какая книга? (необязательно)',
    bookNone: 'Не указана',
    ratingLabel: 'Ваша оценка',
    ratingHint: 'Поставьте оценку',
    ratingNames: { 1: 'Плохо', 2: 'Слабо', 3: 'Нормально', 4: 'Хорошо', 5: 'Отлично' },
    starAria: (n) => `Оценка ${n} из 5`,
    titleLabel: 'Заголовок (необязательно)',
    titlePlaceholder: 'Именно та раскладка, которая нужна',
    bodyLabel: 'Ваш отзыв',
    bodyPlaceholder: 'Для чего использовали? Что удобно, что нет? Только обычный текст.',
    bodyHint: (min, max) => `Обычный текст, ${min}–${max} символов.`,
    bodyCounter: (chars, words, max) => `${chars}/${max} символов · слов: ${words}`,
    photoLabel: 'Ссылка на фото (необязательно)',
    photoPlaceholder: 'https://…',
    photoHint: 'Вставьте прямую ссылку на изображение (https). Загрузка файлов не нужна.',
    consentLabel: 'Согласен(на) на публикацию отзыва на этом сайте с указанием имени и города.',
    submit: 'Отправить отзыв',
    sending: 'Отправляем…',
    cancel: 'Отмена',
    closeLabel: 'Закрыть форму отзыва',
    errorBanner: 'Поправьте отмеченные поля.',
    errors: {
      required: 'Заполните это поле.',
      body_short: `Чуть подробнее, пожалуйста — минимум ${REVIEW_MIN_LENGTH} символов.`,
      body_long: `Слишком длинно — уложитесь в ${REVIEW_MAX_LENGTH} символов.`,
      photo_invalid: 'Нужна полная ссылка на картинку, начинающаяся с https://.',
      consent_required: 'Отметьте согласие на публикацию.',
      too_fast: 'Слишком быстро — пожалуйста, помедлите секунду и отправьте снова.',
      bot: 'Что-то пошло не так. Попробуйте ещё раз.',
    },
    successTitle: 'Спасибо — отзыв получен',
    successBody: 'Отзыв отправлен владельцу сайта. Он появится здесь после проверки и публикации — обычно в течение дня-двух.',
    errorTitle: 'Не удалось отправить отзыв',
    errorBody: 'Соединение не сработало. Можно повторить попытку или отправить тот же текст письмом на адрес владельца.',
    retry: 'Повторить',
    mailTitle: 'Почти готово — отправьте письмом',
    mailBody: 'У сайта нет сервера отзывов, поэтому текст ниже уже готов как письмо. Откройте его в почтовом приложении или скопируйте и отправьте сами.',
    mailCta: 'Отправить письмом',
    copyCta: 'Скопировать текст отзыва',
    copiedCta: 'Скопировано',
    copyFailed: 'Скопировать не удалось — выделите текст ниже и скопируйте вручную.',
    done: 'Готово',
    letterLabel: 'Отзыв в виде письма',
    letterHint: 'Получатель',
  },
};
